...
def find_optimal_k_v2(data, k_min=3, k_max=None, epsilon=1e-10, step=1):
    """
    Determine the optimal value of k for computing fractal dimensions.
    ...
    # If k_min is less than 10, set it to 12.5% of the dataset size (rounded)
    if k_min < 10:
        k_min = math.ceil(0.125 * n_samples)  # 12.5% of the total number of elements

    if k_max is None:
        k_max = math.ceil(0.125 * n_samples)  # Default to 12.5% of the dataset size
...
#def compute_fractal_dimension(data, k, epsilon=1e-10):
...
if __name__ == "__main__":
    ...
    # Find the optimal nearest-neighbor 
    #k = find_optimal_k(data, k_min=2, k_max=15)
    #k = optimal_k_with_elbow_method(data)
    #k = find_optimal_k_v2(data, k_min=10, k_max=25)
    #k = args.k
